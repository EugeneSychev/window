...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; //obj в который записываются заполненые данные

  var deadline = '2022-04-19';
...
}); //DOMContentLoaded - scripts начнут выполняться после построения структуры
...
var changeModalState = function changeModalState(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width'); //проверка на числа
...
var checkNumInputs = function checkNumInputs(selector) {
  ...
  numInputs.forEach(function (item) {
    item.addEventListener('input', function () {
      item.value = item.value.replace(/\D/, ''); //ф, проверка что вписываются цифры
    });
  });
};
...
var forms = function forms(state) {
  ...
  var message = {
    loading: "Загрузка",
    success: "Спасибо! Ближайшее время мы перезвоним!",
    failure: "Что-то пошло не так..."
  };

  var postDate = function postDate(url, data) {
    var res;
    return regeneratorRuntime.async(function postDate$(_context) {
      while (1) {
  ...
  var clearInputs = function clearInputs() {
    inputs.forEach(function (item) {
      item.value = ''; //очищаем значение инпутов
    });
  };

  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); //отмена стандартного поведения браузера при отправке(перезагр.)

      var statusMessage = document.createElement('div'); //создаем элемент с сообщением

      statusMessage.classList.add('status');
      item.appendChild(statusMessage); //помещаем в конец формы 

      var formDate = new FormData(item); //собираем все данные с формы
      ...
      postDate('assets/server.php', formDate) //отправляем данные по адресу сервера
      .then(function (res) {
        console.log(res);
        ...
        clearInputs(); //ф очистки инпутов

        setTimeout(function () {
          statusMessage.remove();
        }, 5000); //удаление сообщения после показа 
      });
    });
  });
};
...
var images = function images() {
  ...
  workSection = document.querySelector('.works'),
      bigImage = document.createElement('img'); //создаем большое изображение

  imgPopup.classList.add('popup'); //добавляем модалке класс

  workSection.appendChild(imgPopup); //добавляем массиву ф-нал

  imgPopup.style.justifyContent = 'center';
  imgPopup.style.alignItems = 'center';
  imgPopup.style.display = 'none'; //скрываем блок

  imgPopup.appendChild(bigImage); //добавляем в модалку img

  workSection.addEventListener('click', function (e) {
    e.preventDefault(); //сброс настроек поумолчанию

    var target = e.target;

    if (target && target.classList.contains('preview')) {
      //проверяем объект события и есть ли определенный класс у этого объекта события
      imgPopup.style.display = 'flex';
      var path = target.parentNode.getAttribute('href');
      bigImage.setAttribute('src', path); //через setAttribute заменяем путь до img
    }

    if (target && target.matches('div.popup')) {
      //условия закрытия модалки
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        //массив кнопок и ссылок
    modal = document.querySelector(modalSelector),
        close = document.querySelector(closeSelector),
        windows = document.querySelectorAll('[data-modal]'),
        //дата атрибут на все модалки, для закрытия их всех
    scroll = calcScroll();
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        document.body.style.marginRight = "".concat(scroll, "px"); //отступ для скролла
        // document.body.classList.add('modal-open'); //realise through classes bootstrap
    ...
    }); //функция закрытия при клике на подложку
  }
  ...
  function calcScroll() {
    //ф подсчета ширины скролла
    var div = document.createElement('div'); //создаем блок

    div.style.width = '50px';
    div.style.height = '50px';
    div.style.overflowY = 'scroll'; //всегда показывать полосы прокрутки

    div.style.visibility = 'hidden'; //скрываем блок

    document.body.appendChild(div); //помещаем его на страницу

    var scrollWidth = div.offsetWidth - div.clientWidth; //вычисляем ширину скролла

    div.remove(); //удаляем блок после подсчета 
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  var header = document.querySelector(headerSelector),
      //блок кот объединяет все табы
  tab = document.querySelectorAll(tabSelector),
      content = document.querySelectorAll(contentSelector); //html коллекция в кот все файлы под определным селектором

  function hideTabContent() {
    content.forEach(function (item) {
      item.style.display = 'none';
    }); //ф скрывает весь контент переданный по селектору
  ...
  function showTabContent() {
    ...
    content[i].style.display = display; //ф отслеживает на какой элемент щелкнул пользователь
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endtime) {
    // ф подсчета времени до deadline
    var t = Date.parse(endtime) - Date.parse(new Date()),
        //через new Date() записываем текущю дату
    seconds = Math.floor(t / 1000 % 60),
        //делим с остатком,получаем секунды
    minutes = Math.floor(t / 1000 / 60 % 60),
        hours = Math.floor(t / (1000 * 60 * 60) % 24),
        days = Math.floor(t / (1000 * 60 * 60 * 24));
    return {
      //вернем вычисленные данные 
      'total': t,
      'days': days,
      'hours': hours,
      'minutes': minutes,
  ...
  var setClock = function setClock(selector, endtime) {
    //ф помещает значения в элементы
    var timer = document.querySelector(selector),
        days = timer.querySelector('#days'),
        hours = timer.querySelector('#hours'),
        minutes = timer.querySelector('#minutes'),
        seconds = timer.querySelector('#seconds'),
        timeInterval = setInterval(updateClock, 1000); //обновление таймера каждую секунду.задали через setInterval чтобы ее остановить через clearInterval

    updateClock();

    function updateClock() {
      //ф пересчета и обновления данных 
